module.exports=function registerOwnerAccountsV2(app,io,db){
 ...
 function accountState(type){
  ...
  `).all(type);
  const movementBalance=rows.reduce((sum,row)=>
   sum+(row.type==='IN'?number(row.amount):-number(row.amount)),0
  );
  const balance=number(account?.opening_balance)+movementBalance;
  return {
   balance,
 ...
 function replaceAccount(type,state,{setOpeningOffset=false}={}){
  ...
  transactions.forEach((transaction,index)=>{
   ...
   insert.run(
    ...
    transaction?.balanceAfter===null||transaction?.balanceAfter===undefined?null:number(transaction.balanceAfter)
   );
  });
 }
